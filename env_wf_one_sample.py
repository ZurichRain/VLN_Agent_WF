...
class EnvBatch():
    ...
    def getStates(self):
        ...
        for i, sim in enumerate(self.sims):
            ...
            if self.features:
                feature = self.features[long_id] # get this scanid's viewpoint image feature
    ...
    def reset(self, batch=None, inject=False, **kwargs):
        ...
        if batch is None:       # Allow the user to explicitly define the batch
            ...
            if inject:          # Inject the batch into the next minibatch
                ...
                self.batch = batch
        scanIds = [item['scan'] for item in self.batch] # scanid in each sample
        viewpointIds = [item['path'][0] for item in self.batch] # first view point in each sample
        headings = [item['heading'] for item in self.batch] # heading stat in each sample
        self.env.newEpisodes(scanIds, viewpointIds, headings) # according to each scan construct a virtual agent point
