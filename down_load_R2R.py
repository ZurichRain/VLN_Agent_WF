...
def get_release_scans(release_file):
    ...
    scan_lines = requests.get(release_file).text.encode('utf8')
    print(scan_lines)
    # scan_lines = scan_lines.encode('ascii')
    html = scan_lines.text
    bs = BeautifulSoup(html,'html.parser')
    pres = bs.find_all('pre',class_='')
    print(pres) 
    scans = []
    for scan_line in scan_lines.readlines():
...
def download_scan(scan_id, out_dir, file_types):
    ...
    for ft in file_types:
        ...
        # http://kaldir.vc.in.tum.de/matterport/v1/scans/Z6MFQCViBuw/matterport_skybox_images.zip
